...
// HTML-modellens kopia: enda skillnaden mot wineBarRoom.ts är den här
// import-raden och att typerna är strippade. Logiken är identisk.
import * as THREE from 'https://unpkg.com/three@0.160.0/build/three.module.js';

// Typerna är strippade — se wineBarRoom.ts för kontraktet.
...
export function disposeWineBarGeometry() {
 ...
 */
export function createWineBarRoom(options) {
  const opts = options ?? {};
  ...
  function tableBox(id, x, z, lx, lz) {
  ...
  const entrance = [halfW - 0.6, 0];
  const waitingSpot = [halfW + 2.5, 0];
 ...
 */
export function updateWineBarRoom(room, phase) {
 ...
 */
export function measureWineBarRoom(room) {
 ...
 * körs.
 */
export function checkSightLines(room) {
  room.group.updateWorldMatrix(true, true);
  ...
  function clearTo(node, from, allow) {
