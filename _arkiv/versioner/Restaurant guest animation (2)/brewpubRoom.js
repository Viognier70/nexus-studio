...
// Typerna är strippade — se brewpubRoom.ts för kontraktet.
...
export function disposeBrewpubGeometry() {
 ...
 */
export function createBrewpubRoom(options) {
  const opts = options ?? {};
  ...
  function slabPlate(parent, material,
                     w, d, x, z, name) {
  ...
  for (let li = 0; li < longSpec.length; li++) {
    ...
    for (let side = 0; side < 2; side++) {
      ...
      for (let k = 0; k < 2; k++) {
        ...
        const perimLane = L.z < 0 ? 'perimS' : 'perimN';
 ...
 */
export function updateBrewpubRoom(room, phase) {
 ...
 */
export function measureBrewpubRoom(room) {
