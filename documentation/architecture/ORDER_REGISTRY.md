...
| 039 | Building Overlap Diagnostic | Executed 2026-07-30 — diagnostic report; no data changed | **Own document** | `ORDER_039_BUILDING_OVERLAP_DIAGNOSTIC.md`. Parent: `PHASE_IV_PRODUCTION_PLAN.md` (visual acceptance criterion); `ADR_001_DIGITAL_TWIN_PHASE.md`. Executed 2026-07-30: full report at `documentation/architecture/BUILDING_OVERLAP_DIAGNOSTIC_REPORT_ORDER_039.md`; register summary at `documentation/world/APPROXIMATION_REGISTER.md` (2026-07-30 ORDER 039 entry). Findings: 39 tier-3 substantial building overlaps including 8 intrusions into `gry-kyrka`; root cause is `scripts/densify-villagerings.mjs` (cites *"ORDER 032"* which is a fifth provenance gap not previously catalogued by ORDER 035 §2.2); 87 handcrafted `vw-*` buildings in `world.json`, none marked SYNTHESISED per ADR 002 §2.3, most carrying invented addresses without source. Refers three items to Vision Owner (§7 of report). Zero changes under `frontend/src/strategic/data/`. |
