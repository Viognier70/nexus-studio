...
| 043 | The Capital Wager and the Consequence Chain | Awaiting Vision Owner approval | **Own document** | `ORDER_043_CAPITAL_WAGER_AND_CONSEQUENCE_CHAIN.md`. Parent: `ORDER_042_BUILD_FIRST_PLAYABLE_LOOP.md` §3.5. Motivation: 2026-08-08 playtest of ORDER 042 established the loop is not yet a game — choice A dominated (no stake) and the visual consequence was one puck two metres away (no consequence in the room). This order adds the wager (§4) and the consequence chain (§6). Five capitals per `GAME_DESIGN_CONSTITUTION.md` become the game's mechanics rather than a document list; wager placed in the room on capital states, not on theme labels (§6 Vision Owner addition 2026-08-08 — the wager UI must not be a menu with different graphics); theme selection weighted by weakness with damping (§4). ORDER 042 §3.3 difficulty selector is removed. Cycle-1 narrowing (Vision Owner 2026-08-08): three themes (social, ecological, economic), one scenario per theme, one consequence event per capital; cultural + scientific capitals stay in the schema without scenarios. Acceptance §8: Vision Owner plays four scenarios in sequence and reports (1) real wager decision, (2) one loss shaping a later scenario, (3) no dominant response. |
