...
**Status:** Living document. Original proposal filed under ORDER 063 (2026-08-12); status rows reconciled against commits + test files under ORDER 084 (2026-08-13); reconciled against the whole-game schema (Miro board `uXjVIGbwLcE`, printed 2026-08-14) under ORDER 091 (2026-08-14). Autonomous DoDs on each landed milestone reference the commit hash and test file that verified them.
**Version:** 1.2 (2026-08-14 — ORDER 091 schema reconciliation)
**Author:** Claude Code, under ORDER 063; ORDER 084 audit and status corrections by Claude Code; ORDER 091 schema reconciliation by Claude Code.
**Scope:** The strategic view of Grythyttan + the restaurant service simulation running on top of it. Everything ORDER 043–062 has touched *except* the first-person prologue (VERTICAL_SLICE_001), which is treated separately in §5. **Under ORDER 091 the scope widens to the full loop of `SPELSLINGAN_SCHEMAT.md`** (the game's whole from paviljongerna → bankmöte → verksamhetsval → satsningar → service → utfall → quizer → nästa varv) — M0–M8 still describe the satsningar-och-service *middle* of that loop; the ryggrad milestones **R1–R7** in §1.5 cover the rutor the middle sits between.
...
**ORDER 091 schema reconciliation summary (2026-08-14).** The M0–M8 list was written against the strategic slice as-executed, not against the game's whole loop. When `SPELSLINGAN_SCHEMAT.md` landed (2026-08-14, from the Miro board `uXjVIGbwLcE` + Vision Owner precisering §3) it became clear that M0–M8 cover two rutor of a nine-ruta loop — the satsningar-och-service middle. The ram around that middle (paviljonger, bankmöte, verksamhetsval, action-knapp, quizer efter servicen, omgångsslinga, kunskapskapital som valuta) is largely unbuilt. Under this order the M-list keeps its numbering and its status columns unchanged, but three things move:
1. The §1 table gains a **Ruta i slingan** column mapping every M-milestone to its position in the schemat.
2. A new **§1.5 R1–R7 — Ryggraden** groups the seven backbone milestones the loop needs before M0–M8's middle-of-the-loop work becomes a *game* rather than a well-instrumented service simulator. R1 (kunskapskapital + krediter) blocks everything else in the ryggrad; the underlag at `documentation/blueprints/MILSTOLPAR_MOT_SCHEMAT.md` §4 has the full derivation.
3. Two status reclassifications: **M7b's block is not technical** — the answer-to-loan mapping *is* the game design, not a report-gated report; it moves from a technical blocker to a design-decision milestone slotted alongside R3 in the ryggrad. **M7a's position in the loop is wrong** — the schemat places quizer *after* the service, riktade mot uppvisade svagheter; M7a's chef-questions currently fire *inside* the service and pick up the "research prose mid-service" reading that M8 punch-list row 25 recorded. Both are noted in place in §2, not renumbered.
...
**Ruta i slingan column added under ORDER 091.** Every M-milestone maps to a ruta in `SPELSLINGAN_SCHEMAT.md` §1. Rutor without a matching M-milestone belong to the ryggrad (§1.5, R1–R7). "Fel ruta" flags a semantic mismatch between what the milestone builds and where the schemat places that beat.

| # | Name | Ruta i slingan | Kind | Status | Blocking open threads |
|---|---|---|---|---|---|
| M0 | Village + room baseline | infrastruktur (utanför slingan) | RENDER | ~95 % (2 tail items). Adjacent ORDER 083 pitch-probe measurement filed 2026-08-13 as `M0_CAMERA_PITCH_PROBE_REPORT_ORDER_083.md` — finding: actors are cylinders, pitch is not the bottleneck; two paths (rig the puck / add presentation mode) both ORDER-sized and unopened. | 057 Del D, 061 pt3 fog verification, pitch/actor-legibility decision from ORDER 083 |
| M1 | First playable loop | satsningar + service | MIXED | **Landed 2026-08-13 (commit `bebac5c`); verified via `m1.test.ts`.** DoD 1 (3 days no desync), DoD 3 defect A (valuation math documented in `valuation.ts`), DoD 3 defect C (initial mid-band quality documented) all green. DoD 2 relaxed to state-divergence per proposal §6.2; text divergence deferred to M6. DoD 4 → M8. **DoD 3 Defect B REOPENED 2026-08-13** — original closure only inspected 1280×720; wide-viewport overlap reproduces per M8 pass 1. | Reopened defect B (panel overlap on wide viewport) |
| M2 | Morning legibility (activities visible) | **satsningarna som viktar servicen** | MECHANIC + UI | **Landed 2026-08-13 (commit `609eaf1`); verified via `m2.test.ts`.** DoD 1 (pick 1–3 activities, weekly-gate, cash + ledger flow), DoD 3 (evening account names picked activity), DoD 4 (no card labels its capital) all green. DoD 2 (three-column visual on card) is a DOM assertion — landing commit ships the panel; DOM test lives with the panel test suite. Report gate opened under ORDER 075 (`M2_ACTIVITY_MODEL_REPORT_ORDER_075.md`). Reads stronger against the schemat than the pre-ORDER-091 list gave it credit for — the aktivitets-deltan *är* the satsningsviktningen. | — |
| M3 | Evening ledger visible | **utfall i tre hållbarheter** | MECHANIC + UI | **Landed 2026-08-13 (commit `9855a6c`; ORDER 074 follow-up `48a9d29` closed collapse-close reconciliation hole); verified via `m3.test.ts`.** DoD 2 (every mover posts labelled line, categories revenue/ingredient/wage/interest/scenario present), DoD 3 (overall reconciliation within ±2 %, drift < 1500 SEK on 3-day script), deliverable (ledger persists across day boundaries) all green. DoD 1 (single-click open) satisfied inline — panel embeds ledger table directly under paragraph; zero clicks. Reads stronger against the schemat than the pre-ORDER-091 list gave it credit for — the kvällsbokföringen *är* utfallsrutan. | — |
| M4 | Menu + kitchen + stock | satsningar | MECHANIC + UI | Landed 2026-08-13 (ORDER 077 §4, commit `db639d5`); DoD 1–4 verified via `m4.test.ts`; wine / packages / ingredientTier retirement rolled to M4b | — |
| M5 | Prep + cadence in the room | service (restaurang) | MECHANIC + RENDER | Landed 2026-08-13 (ORDER 078, commit `b21eda2`); DoD 1 + 3 + 4 verified via `m5.test.ts`, DoD 2 mechanic verified (colour-transition sampling); sight-read remains M8. **Note under ORDER 091:** the mise-en-place + rytm subsystem is restaurang-shaped; if R4 introduces food truck as its own verksamhetsklass the mechanic will need a matbryttsanpassad variant (no matsal, no bord, ingen sittande gäst). | — |
| M6 | Cause-aware texture (mechanic) | service | MECHANIC | Landed 2026-08-12 (ORDER 076, commit `0b4eb89`); verified via `m6.test.ts`; divergence 0.219, first-cut floor cleared | — |
| M6b | Cause-aware texture (sentence-bank rewrite) | service, kvällsberättelse | CONTENT | Not started (split out from M6 per ORDER 077) | 052 §9 step 1 |
| M4a | Attractiveness weighting + substitute/walkout split | service | MECHANIC | Landed 2026-08-13 (ORDER 079, commit `e649f2a`; DoD 1 tightened by ORDER 080 `b255cad`); DoD 1 + 1b + 2 verified via `m4a.test.ts` — closes ORDER 051 §8 acceptance items #2 and #4 that M4 as filed cannot satisfy | — |
| M4b | Menu + kitchen + stock (residuals) | satsningar | CONTENT + MECHANIC | Not started (attractiveness split out to M4a per ORDER 078 §8); residuals = wine list, package menus, ingredientTier retirement, stock ageing | 051 §7 steps 5–7 + §4 ageing |
| M7a | Chef service-questions (end-to-end flow verified) | **FEL RUTA — hör till quizrutan (efter service), byggd inuti servicen** | MECHANIC | Landed 2026-08-13 (ORDER 080, commit `84d24dc`); DoD 1 + 2 + 3 verified via `m7a.test.ts` — mechanism was already built, M7a proves it end-to-end. **ORDER 091 note:** in `SPELSLINGAN_SCHEMAT.md` §1 the quiz-rutan sits *after* the service ("Frågor och quizer inom de områden som uppvisade svaghet under servicen"), not inside it. M7a's chef-questions currently fire mid-pass and picked up the "research prose mid-service" reading that M8 punch-list row 25 captured — the tone is likely correct for a post-service quiz surface, wrong for a mid-service interrupt. Move (not delete) is the ORDER-091 recommendation; carried by R6. | Placement: to be moved to R6 (quizer efter servicen) — reopens ORDER 049 §7 step 3 scope |
| M7b | Bank meeting scene | **gångjärnet** (banklånet) | MECHANIC + UI | ~~Blocked pending ORDER 049 §7 step 8 answer-to-loan mapping report~~ — **reclassified under ORDER 091**: the block is not technical. The answer-to-loan mapping *is* the game design (how does kunskapskapital per domän translate to lånebelopp, and where sit the trösklar mellan inget lån / food truck / restaurang?). It's a design decision that needs a report gate before build — same shape as R3 in the ryggrad, and better tracked there. This entry stays as the M-family surface (bank-mötesscenen); the design decision that unlocks it moves to R3. | Design decision at R3, then build here. Not a report-gated technical dependency. |
| M7c | Multi-role question coverage (sommelier / värd / servitör) | quizrutan | CONTENT | Blocked pending three knowledge-generate script runs | 049 §1 volume across roles |
| M8 | Playthrough acceptance | hela slingan (utvidgas under ORDER 091 till att gälla hela varvet, ej bara ett pass) | GOVERNANCE | **In execution — pass 1 completed 2026-08-13 (`#playtest=1` day 1).** Four Vision-Owner findings filed to `M8_PERCEPTION_PUNCH_LIST.md` (rows 22–25). ORDER 081 brief (`M8_PLAYTEST_BRIEF_ORDER_081.md`) is the operating reference. Sign-off deferred pending fixes for the four findings and reopened M1 defect B. **Under ORDER 091 M8's scope widens to the whole loop** — the question is no longer just "does the room read" but "håller varvet från paviljong till nästa satsning". | Four playtest findings + reopened M1 defect B + ryggrad completion; typically ≥ 2 passes before clean sign-off |

**With M1/M2/M3 landed, the "act → see consequence → want to repeat" bar is met end-to-end.** M8 pass 1 opened 2026-08-13; see §3 for what pass 1 surfaced. **After ORDER 091 that bar is understood as a partial acceptance:** the middle of the loop reads; whether the loop is a game (kunskap → lån → verksamhet → satsning → utfall → quiz → nytt varv) is answered by R1–R7 in §1.5.

---

## 1.5 R1–R7 — Ryggraden (ORDER 091)

The ryggrad describes the seven backbone milestones that surround the middle-of-the-loop work in M0–M8 with the rutor from `SPELSLINGAN_SCHEMAT.md` §1 that are currently unbuilt. Numbered in the order they block each other; R1 is the head of the dependency chain and blocks everything else. Full derivation, verksamhetsklass-diskussion, and reordering considerations sit in `documentation/blueprints/MILSTOLPAR_MOT_SCHEMAT.md` §§ 2–6 — this section is the milestone summary of that underlag, so the M-list has one authoritative index.

**Nothing under M0–M8 is rebuilt.** Each ryggrad-milestone touches surfaces that either don't exist yet (R1's kunskapskapital, R2's paviljonger, R5's action-knapp, R6's post-service quizer, R7's omgångsslinga) or exist as assumptions the code is making silently (R4's verksamhetsklass: today the code assumes restaurang; food truck needs an explicit klass ovanför).

| # | Name | Ruta i slingan | Kind | Status | Depends on |
|---|---|---|---|---|---|
| R1 | Kunskapskapital + krediter | valutan under hela slingan | MECHANIC | Not started. Gates every other ryggrad-milestone. | — |
| R2 | Paviljongerna | paviljongerna, återbesökbara | MECHANIC + CONTENT + RENDER | Not started. VS001 (arrival) mergas in här (§5). | R1 |
| R3 | Bankmötet + answer-to-loan-mappningen | banklånet (gångjärnet) | MECHANIC + REPORT GATE | Not started. **Report first, build second** — the mappning kredit-per-domän → lånebelopp och trösklarna mellan inget lån / food truck / restaurang är speldesignen. Absorbs M7b's block; M7b becomes the scen-implementationen efter R3-rapporten är klar. | R1, R2 |
| R4 | Verksamhetsklass | verksamhetsval, med food truck som första implementation | MECHANIC + RENDER | Not started. Introduces the class as a concept ovanför `Restaurant.tsx` / `layout.seats` / `TOTAL_SEATS = 16` (which today implicitly assume restaurang). Food truck first — mindre av de två klasserna, prövar dockskåpsformen på en lucka + kö + väder + gatuläge innan sexton kuvert ska rymmas. | R3 |
| R5 | Action-knappen | händelseförloppet under service (spelaren tar initiativ) | MECHANIC + UI | Not started. Distinct from today's scenariesystem — scenarierna avbryter spelaren med ett val; action-knappen kräver att spelaren själv trycker för att försöka rädda en situation eller tjäna krediter. Det befintliga scenariomaterialet är byggmaterialet; utlösaren är ny. | R1 for krediter som insats |
| R6 | Quizer mot uppvisade svaghet | quizer efter servicen, riktade mot svaga domäner | MECHANIC + CONTENT | Not started. **Absorbs M7a's placement move** — chef-frågorna flyttar hit och får sin kontext (post-service, riktade mot en domän utfallet pekade på, med krediter som insats). Also absorbs M7c (flerrollsfrågorna). | R1, R3 for att utfallet ska kunna peka ut domän |
| R7 | Omgångsslingan | nästa varv (ackumulerat kapital förs vidare) | MECHANIC | Not started. Repot är i dag dagbaserat och kontinuerligt; schemat är omgångsbaserat. Två tidsstrukturer som måste förenas. | R1–R6 |

**Föreslagen ordning (from `MILSTOLPAR_MOT_SCHEMAT.md` §6, unchanged):** R1 → R3 rapportgrind → R2 → R3 build → R4 → R5 → R6 (with M7a + M7c folded in) → R7. SD-003 (dockskåpet) is a pausera-och-revidera against R4 rather than a ryggrad-milestone itself — the current directive describes one shape, R4 requires at least two (restaurang i genomskärning, food truck som en lucka i gatuplan).

**How M0–M8 slots into this.** M0–M4 satsningsverktygen och rummet är byggda mot restaurang-klassen — usable under R4 as-is when the klassen är restaurang, needs a matbryttssyskonvariant when klassen är food truck. M5 mise-en-place + rytm är restaurang-formad. M6/M6b service-texten kan bära båda klasserna. M7a flyttar till R6. M7b's scen byggs efter R3. M7c fold till R6. M8's bar utvidgas till hela varvet (per §1 M8-raden).

---

## 1.6 Two open decisions this reconciliation reopens (ORDER 091)

**ORDER 089 (medgången) — omtolkas, inte rivs.** Rapporten mätte att servicen saknar positiv återkoppling (60 % pressure faces on staff, red rytm 75 % of ticks, `smiling` and `proud` at zero ticks) and lade fram val för att bygga in belöning inuti servicen. Under ORDER 091 the reading is different: belöningen i schemat är krediten spelaren räddar med action-knappen och kapitalet som förs in i nästa varv. Bygg inte en parallell belöningsmekanik inuti servicen — bygg R1 och R5. **Report data står kvar och är värdefull; dess §§2–5 handlingsval bör vänta tills R1/R5 landat.** M9 report §9's two decision fields (9a `proud` reader-path, 9b load-model calibration) står kvar oberoende.

**SD-003 (dockskåpet) — pausas och revideras.** Presentationsbeslutet är riktigt och förfaller inte. Men direktivet beskriver ett dockskåp (en form); R4 kräver att formen håller minst två skepnader som delar renderare men inte planlösning — restaurang i genomskärning med rumsval, food truck som en lucka i gatuplan. Kartan fungerar för båda och visar kö och gata i stället för bord. Revidering är ORDER-sized men inte påbörjad.
...
**ORDER 091 placement finding — belongs to quizrutan, not mid-service.** `SPELSLINGAN_SCHEMAT.md` §1 puts quizerna *after* the service, riktade mot de områden som uppvisade svaghet, med krediter som insats. M7a's chef-frågor fire *inside* the pass and picked up the "research prose mid-service" tone reading that M8 punch-list row 25 recorded. The tone is likely correct for a post-service quiz-yta, wrong for a mid-service interrupt. **Move (not delete)** is the ORDER-091 recommendation — the mechanism, the 272-approved chef corpus and the DoD trail all survive; the trigger surface moves from scenario-mid-pass to R6 (quizer efter servicen). Rescheduling is a scope call for the Vision Owner; R6's DoD (§1.5) includes the migration.
...
**Reclassified under ORDER 091 — the block is not technical.** Previously M7b was filed as "blocked pending ORDER 049 §7 step 8 answer-to-loan mapping report". `SPELSLINGAN_SCHEMAT.md` §3 makes clear the mappning *is* the speldesignen — how does kredit-per-domän översätts till lånebelopp, och var går trösklarna mellan inget lån (tillbaka till paviljongerna och öva), food truck och restaurang? That's a design decision, not a report awaiting technical inputs. It is the **gångjärnet** i hela slingan — allt nedströms (verksamhetsklass, satsningar, service, utfall) hänger på det beslutet.

**The design decision moves to R3 in the ryggrad.** M7b as a milestone stays as the M-family surface (bank-mötesscenen — bank director scene at loan application + post-bankruptcy return loop), and lands *after* R3 delivers the answer-to-loan mapping decision. If the Vision Owner opens R3's report gate, R3 answers the mapping question and M7b then builds the scen on top. This is a rename of the dependency, not a new blocker: the same design conversation must happen; the ORDER-091 reclassification moves it out of the "waiting on a technical report" category (where it can drift indefinitely) into the ryggrad-order (where it's understood as the game's central decision).

**Deliverables:**
- Answer-to-loan mapping decision — moved to R3
- Bank director scene at loan application — stays M7b
- Loan tier decision from composite answer scoring — stays M7b, consumes R3's mapping
- Bankruptcy return loop (ORDER 049 §5) — stays M7b
...
- **New under ORDER 091 (ryggrad ordering).** The R1–R7 order in §1.5 is a dependency-derived proposal (R1 blocks the rest; R3 must open its report gate before R3 builds; R6 waits on R1 and R3's utfall-pointing). The Vision Owner's call is whether that ordering stands, or whether an earlier milestone in the list should be opened first for playtesting reasons (e.g. paviljongerna as a standalone learning surface before the whole loop is threaded).
- **New under ORDER 091 (VS001 merge trigger reopened).** §5's recommendation was "merge VS001 → strategic at the end of M4." Under the schemat reading, VS001 (bussankomst → NPC → registrering vid Sevillapaviljongen) *is* the ankomst-till-paviljongen boxen. That reroutes the merge: rather than merging VS001 into the strategic loop at end-of-M4, VS001 becomes the way into R2 (paviljongerna). The recommendation stands; the trigger shifts from M4 to R2.
- **New under ORDER 091 (M7a placement move).** M7a landed; its DoD passed. But `SPELSLINGAN_SCHEMAT.md` places the quizerna post-service. Does the Vision Owner accept the migration recommendation (move the chef-frågor surface to R6), or keep them mid-service and revise the schema-reading?
- **New under ORDER 091 (R3 as a gate).** M7b's block being "the mappning is the game design" (not a technical dependency) implies R3 needs an authored report gate before it builds — same pattern as the M6b prose gate. Does the Vision Owner want R3's mapping report opened as the next design decision after R1's kunskapskapital-model?
- The three-report-gate stack that this section originally listed (M3 first, then M2 unblocking on its evening-account tie-in) is closed: M2 and M3 both landed 2026-08-13. Remaining M-family gates are M6b (sentence-bank rewrite) and M7c (three additional knowledge-generate runs for sommelier / värd / servitör). Under ORDER 091 M7b's gate is reclassified from technical-report to design-decision-at-R3. Which of the M6b / M7c / R1 / R3-report does the Vision Owner open next?
- The four M8 pass-1 findings (§3) each require touching landed code (M1 for panel layout — closed under ORDER 090 §6; M4 for open-with-empty-stock; M1/M3 for reputation-floor recovery; M7a/M6b for chef-question tone — R6 subsumes the M7a half of this if the migration is accepted). Should each ship as its own ORDER or bundled into the next M8-preparation pass?
