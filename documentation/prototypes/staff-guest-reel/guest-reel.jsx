...
function sitHail(T) {
  ...
  return Object.assign({}, SIT, { lean: 1, head: -4, armNear: [166, -14 + 16 * w], armFar: [58, -60] });
}
function sitWait(T) {
...
function Props({ T, C }) {
  ...
  const plateDrop = MOTION.pop(-260, 0, C.Serve + 0.35, C.Serve + 1.1)(T);
  ...
  return (
    <g>
      ...
      <g opacity={plateOp} transform={`translate(0,${plateDrop})`}>
...
function Piece({ tw }) {
  ...
  const camX = MOTION.drift([
    ...
    [C.Wait, 1170], [C.Serve, 1230], [C.Leave, 1200], [C.Sheet - 0.3, 1500],
  ])(T);
  const camY = MOTION.drift([
    [0, 545], [C.Arrive, 555], [C.Seat, 600], [C.Order, 585],
    [C.Wait, 585], [C.Serve, 610], [C.Leave, 600], [C.Sheet - 0.3, 560],
  ])(T);
  const camZ = MOTION.drift([
    [0, 0.98], [C.Arrive + 0.4, 1.02], [C.Seat + 0.6, 1.3], [C.Order + 1.2, 1.5],
    [C.Wait + 1.5, 1.62], [C.Serve + 1.2, 1.42], [C.Leave + 0.6, 1.05], [C.Sheet - 0.3, 1.0],
  ])(T);
