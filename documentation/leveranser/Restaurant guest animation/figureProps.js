...
// HTML-modellens kopia: enda skillnaden mot figureProps.ts är den här
// import-raden och att typerna är strippade. Logiken är identisk.
import * as THREE from 'https://unpkg.com/three@0.160.0/build/three.module.js';

// Typerna är strippade — se figureProps.ts för kontraktet.
 ...
 */
export function checkPropPalette(floorColours, minRatio, maxRatio) {
...
export function propContrastRange(floorColours, garmentColour) {
  let fLo = Infinity;
  let fHi = 0;
  let gLo = Infinity;
  const keys = Object.keys(PROP_COLOURS);
...
export function disposeFigurePropGeometry() {
 ...
 */
function shell(r, phiStart, phiEnd, seg) {
...
function ring(inner, outer, h, seg) {
...
function makeHandle(group, mats) {
  return {
    group: group,
    materials: mats,
    dispose: function () {
...
function mesh(parent, geo, m, x, y, z, name) {
...
function stdMat(colour, rough, metal) {
 ...
 */
export function createHandProp(id, options) {
  const g = new THREE.Group();
  g.name = 'handProp_' + id;
  const mats = [];
  const use = function (colour, rough, metal) {
 ...
 */
export function createHeadTopping(id, options) {
  const o = options ?? {};
  ...
  function hairBand(colour, thickness, phiEnd, name) {
  ...
  function onSkull(a, phi, thickness) {
 ...
 */
export function attachProps(rig, spec) {
  const out = [];
  const opts = {
 ...
 */
export function checkCrownCoverage(rig, garmentColour, samples) {
  samples = samples ?? 240;
...
export function measureProp(handle) {
