...
// HTML-modellens kopia: enda skillnaden mot restaurantRoom.ts är den
// här import-raden och att typerna är strippade. Logiken är identisk.
import * as THREE from 'https://unpkg.com/three@0.160.0/build/three.module.js';

// Typerna är strippade — se restaurantRoom.ts för kontraktet.
...
export function disposeRestaurantGeometry() {
...
export function createRestaurantRoom(options) {
  const opts = options ?? {};
  ...
  function put(parent, geo, m, x, y, z, name) {
  ...
  function floorPlate(parent, m, w, d, x, z, y, name) {
  ...
  const entrance = [halfW - ENTRANCE_INSET_M, 0];
  const waitingSpot = [halfW + WAITING_STANDOFF_M, 0];
  const deliveryBay = [-halfW - DELIVERY_BAY_OFFSET_M, 0];
  ...
  const arrivalSlots = ARRIVAL_SLOT_ANGLES.map(function (theta) {
    return [
 ...
 */
export function updateRestaurantRoom(room, phase) {
...
export function measureRestaurantRoom(room) {
 ...
 */
export function checkSeatContract(room) {
  let orderOk = true;
  let seenBar = false;
