...
// Typerna är strippade — se innRoom.ts för kontraktet.
...
export function eyeHeightForSeat(seat) {
 ...
 */
export function checkPaletteAgainstFloors(minRatio, maxRatio) {
  minRatio = minRatio ?? 1.8;
  maxRatio = maxRatio ?? 3.6;
  const fails = [];
...
export function disposeInnGeometry() {
 ...
 */
export function createInnRoom(options) {
  const opts = options ?? {};
  ...
  function put(parent, geo, material,
               x, y, z, name) {
  ...
  function floorPlate(parent, material,
                      w, d, x, z, y,
                      name) {
  ...
  const entrance = [0, inZ - 0.6];
  const waitingSpot = [0, courtZ0 + 2.5];
  const courtyardGate = [0, courtZ1 + 0.5];
  const deliveryDoor = [-halfW - 0.6, -6.5];
  ...
  function rectTable(id, x, z, lx, lz) {
    ...
    const corners = [[-legX, -legZ], [legX, -legZ], [-legX, legZ], [legX, legZ]];
  ...
  const salIn = [[SAL_DOOR_X, SPINE_Z], [SAL_DOOR_X, SAL_LANE_Z]];
  for (let side = 0; side < 2; side++) {
    ...
    for (let k = 0; k < SAL_SEAT_DX.length; k++) {
      ...
      const near = side === 0;
      const approach = near ? [sx, sz + 0.85] : [sx, sz - 0.85];
      const via = near
 ...
 */
export function updateInnRoom(room, phase) {
 ...
 */
export function walkPathToSeat(room, seatId) {
  const seat = room.seats.find(function (s) { return s.id === seatId; });
  if (!seat) return [];
  const path = [[room.entrance[0], room.entrance[1]]];
...
export function exitPathFromSeat(room, seatId) {
 ...
 * den gör inte det.
 */
export function walkPathFromRoom(room, roomId) {
  const gr = room.guestRooms.find(function (r) { return r.id === roomId; });
  if (!gr) return [];
  const path = [[gr.local[0], gr.local[1]],
 ...
 */
export function walkHeightsFromRoom(room, roomId) {
  const gr = room.guestRooms.find(function (r) { return r.id === roomId; });
  if (!gr) return [];
 ...
 */
export function measureInnRoom(room) {
 ...
 */
export function checkRoomCount(room) {
  const perWing = {};
 ...
 */
export function resolveWorldPositions(room) {
  room.group.updateWorldMatrix(true, true);
