...
- **ORDER 175** — DevPanel-raden visade endast `cam=<X>m` utan pil eller target. En enda siffra utan indikator på pågående flygning tolkades i tolv skärmdumpar som "kameran står på detta målvärde", medan bilden visade en helt annan vy (kameran ännu inte dampad, eller mid-flight). Kostade två dygns felsökning där rot-orsaken antogs vara render-buggar. Åtgärd: cam-strängen visar nu `<actual>m→<target>m` när `|actual − target| > 0,5`. Trettonde fallet av "rätt tal om fel sak" — talet var korrekt actualRef.distance, men UI:et gav ingen väg att skilja "landad" från "på väg dit".
...
7. **Svenska i spelet (Nexus v1), engelska i koden.** `NEXUS_SPELDESIGN_V1.md` > Språk och målgrupp: "Spelet är på svenska i version 1." Det upphäver Observation 6 (2026-08-09, engelsk spelartext) — se `NEXUS_V1_OPPNA_FRAGOR.md` F9. Ny spelartext skrivs på svenska via strängfilerna, aldrig hårdkodad i komponenter. Befintlig engelsk spelartext översätts i en egen omgång; tills dess är gränssnittet blandat. Frågor skrivs med spelartext och metadata separerade så att engelska kan läggas till senare. Kod, identifierare och kodkommentarer får vara på engelska eller svenska som i omgivande kod.
