...
// HTML-modellens kopia: enda skillnaden mot figureRig.ts är den här
// import-raden (repot löser 'three' via bundlern) och att typerna är
// strippade. Logiken är identisk.
...
export function disposeFigureGeometry() {
...
function limb(key, material, radius, length, taper) {
...
export function createFigureRig(options) {
  const opts = options ?? {};
...
export function disposeFigureRig(rig) {
...
function applyArm(shoulder, elbow, arm, side) {
  const a = arm ?? {};
  shoulder.rotation.set(-(a.swing ?? 0), 0, side * (a.lift ?? 0));
  elbow.rotation.x = -(a.elbow ?? 0);
}

function applyLeg(hip, knee, ankle, leg, side) {
  const l = leg ?? {};
...
export function blendPose(a, b, k) {
  ...
  function n(x, y) { return (x ?? 0) + ((y ?? 0) - (x ?? 0)) * m; }
  function arm(x, y) {
    const p = x ?? {};
    const q = y ?? {};
 ...
 */
export function poseWalk(phase, options) {
  const o = options ?? {};
...
function kneeIdle(shift) {
  return 0.06 + 0.05 * Math.max(0, shift);
}

function kneeSwing(w, k) {
 ...
 */
export function poseIdle(t, options) {
  const o = options ?? {};
  const k = o.intensity ?? 1;
  const time = t ?? 0;
 ...
 */
export function poseSeated(t, options) {
  const o = options ?? {};
  const time = t ?? 0;
 ...
 */
export function poseGreet(t, options) {
  const o = options ?? {};
  const time = t ?? 0;
  const yaw = o.targetYaw ?? 0;
  const wave = Math.sin(time * 2 * Math.PI * 1.6);
  const raised = { swing: 1.12, lift: 0.52 + 0.20 * wave, elbow: 0.82 - 0.16 * wave };
  const resting = { swing: 0.06, lift: 0.06, elbow: 0.22 };
  let armL = resting;
  let armR = raised;
 ...
 */
export function poseWork(t, options) {
  const o = options ?? {};
  const k = o.intensity ?? 1;
  const time = t ?? 0;
 ...
 */
export function poseCarry(t, options) {
  const o = options ?? {};
  const time = t ?? 0;
 ...
 * bredd = axelbredden).
 */
export function measureFigure(rig) {
