 ...
 */
// ORDER 142-avvikelse: `options` prefixad med _ (unused parameter — TS6133).
// Handoff-kontraktet exponerar signaturen för framtida bruk; ta bort _ när
// PropOptions faktiskt läses av implementationen.
export function createHandProp(id: HandPropId, _options?: PropOptions): PropHandle {
...
export function createHeadTopping(id: HeadToppingId, options?: PropOptions): PropHandle {
  ...
  function hairBand(colour: string, thickness: number, phiEnd: number, name: string) {
    ...
    // ORDER 142-avvikelse: mesh() returnerar THREE.Mesh vars `.material`
    // är `Material | Material[]`; call-site vet att det är enkel Material.
    (sh.material as THREE.Material).side = THREE.DoubleSide;
    return m;
  }
  ...
  if (id === 'ruffled') {
    ...
    // ORDER 142-avvikelse: samma casting-motiv som hairBand() ovan.
    (back.material as THREE.Material).side = THREE.DoubleSide;
