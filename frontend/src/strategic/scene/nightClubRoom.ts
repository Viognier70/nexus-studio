...
export function createNightClubRoom(options?: NightClubOptions): NightClubRoom {
  ...
  const areaDefs: { id: string; kind: any; bounds: [number, number, number, number];
    ...
    {
      // Remsan framför huvudbaren, INTE hela väggen. Första versionen
      // sträckte den från vägg till vägg och gav 61 m² — 43 personer
      // där jag räknat 15, och kapacitetskontraktet gick sönder.
      id: 'barFront', kind: 'barFront', bounds: [-5.0, 5.0, -6.7, -4.5],
      density: 0.70, standHeight: 0,
      note: 'Remsan framför huvudbaren, tio meter av tolv. Kö och genomgång i samma yta.'
    },
    {
      id: 'loungeEdge', kind: 'loungeEdge', bounds: [platX0, platX0 + 11.4, platZ - 1.8, platZ - 0.4],
      density: 0.50, standHeight: PLATFORM_H,
      note: 'Plattformens framkant. Man står vid bordet snarare än sitter.'
    },
    {
      id: 'entryHall', kind: 'entryHall', bounds: [7.4, 12.2, -3.1, 3.1],
      density: 0.64, standHeight: 0,
