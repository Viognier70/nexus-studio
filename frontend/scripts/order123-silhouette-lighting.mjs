#!/usr/bin/env node
// ORDER 123 §4.5 — visuell verifikation att silhuetten läses i både
// dagsljus (lunch) och kvällsljus (evening). Följer samma mönster som
// ORDER 121 §8 DoD 8: baseline utan gäster + sample med gäster,
// diff-mätning isolerar figurbidraget från bakgrunden.
//
// Nytt jämfört med ORDER 121: två körningar, en per lighting-period,
// för att visa att figurerna läses i båda ljusförhållandena. Kontrast-
// bandet självt är hävdat i `silhouetteContrast.ts` + tester — detta
// script är den visuella bekräftelsen.
...
const REPORT_DIR = resolve(FRONTEND_ROOT, 'reports', 'order123');
...
async function measurePeriod(browser, url, period, label) {
  console.log(`\n[order123] === ${label} (period=${period}) ===`);
  const page = await browser.newPage({ viewport: VIEWPORT });
  try {
    // #period-hooken från testHarness/urlParams.ts tvingar dagsljus/kvällsljus
    // via DayLighting-komponenten oavsett sim-state.
    await page.goto(`${url}/#playtest=1&business=restaurant&preset=business&period=${period}`, { waitUntil: 'domcontentloaded' });
    ...
    // Baseline utan gäster.
    console.log(`[order123] baseline utan gäster ...`);
    const baseline = await readFigureRegion(page);
    const baselinePath = resolve(REPORT_DIR, `scene-baseline-${period}.png`);
    await page.screenshot({ path: baselinePath });

    // Injicera 6 gäster.
    console.log(`[order123] injicerar 6 gäster ...`);
    await seedGuests(page, 6);
    const sample = await readFigureRegion(page);
    const samplePath = resolve(REPORT_DIR, `scene-with-bodies-${period}.png`);
    await page.screenshot({ path: samplePath });

    const diff = diffFigureContribution(baseline, sample, 25);
    console.log(`[order123] ${label} figur-diff: ${diff.changed}/${diff.totalPixels} pixlar, bbox ${diff.bboxW}×${diff.bboxH}`);
    console.log(`[order123] skärmdumpar: ${baselinePath} + ${samplePath}`);
...
async function main() {
  ...
  if (failed) {
    console.error('\n[order123] DoD 5 misslyckades');
    process.exit(1);
  }
  console.log('\n[order123] DoD 5 OK — silhuetten läses i både dagsljus och kvällsljus');
}
