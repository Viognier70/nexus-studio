#!/usr/bin/env node
// ORDER 266 (Nexus v1 etapp 4) — servicen i spelarens vy: rycka in för en
// gäst som är på väg att gå, och se det nämnas i strömmen.
//
...
// Utdata: reports/order266/service-playthrough.json + skärmdumpar.
...
const OUT = resolve(FRONTEND, 'reports/order266');
...
async function tryIntervene(tag) {
  ...
  while (Date.now() < deadline) {
    ...
    if (risk) {
      ...
      await risk.dispatchEvent('click');
      await page.waitForSelector('[data-testid=blind-overlay]', { timeout: 5000 });
